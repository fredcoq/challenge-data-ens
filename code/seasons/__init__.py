...
class solstice:
    ...
    def is_winter(self):
        ...
        :return (boolean):
        """
        if (self.my_date.shift().hour == 1) & ((self.my_date.date().strftime("%d/%m") >= "20/12") | (self.my_date.date().month <= 3)):
            return True
        else:
            return False
    ...
    def __set_offpeakhours(self):
        ...
        for d in list_date:
            if HoursRate(d).is_offpeak():
                ...
                self.offpeak_df.at[d, 'offpeak'] = True
            else:
                self.offpeak_df.at[d, 'peak'] = True
